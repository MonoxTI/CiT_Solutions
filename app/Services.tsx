  ...
  return (
    <section id="services" style={{ padding: "6rem 0", background: "#000", backgroundImage: "radial-gradient(circle, rgba(255,255,255,0.03) 1px, transparent 1px)", backgroundSize: "32px 32px" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 2.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }} className="svc-grid">
          {categories.map(cat => {
            const isOpen = active === cat.id;
            return (
              <div key={cat.id} onClick={() => setActive(isOpen ? null : cat.id)}
                style={{ borderRadius: 3, padding: "20px 18px", border: `1px solid ${isOpen ? "#4A4A4A" : "#1E1E1E"}`, background: isOpen ? "#111" : "#0A0A0A", cursor: "pointer", transition: "all 0.25s" }}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ fontFamily: "Space Mono,monospace", fontSize: "0.6rem", color: "#404040", letterSpacing: "0.1em" }}>{cat.num}</span>
                  <svg style={{ transition: "transform 0.3s", transform: isOpen ? "rotate(45deg)" : "none" }} width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M6 1v10M1 6h10" stroke="#555" strokeWidth="1.2" strokeLinecap="round" />
                  </svg>
                </div>
                <h3 style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: "0.95rem", color: "#fff", marginBottom: 6 }}>{cat.title}</h3>
                ...
                {isOpen && (
                  <ul style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #1E1E1E", listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
                    {cat.services.map(s => (
